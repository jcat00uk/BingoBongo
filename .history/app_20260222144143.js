...
autoCheckToggle.onchange = () => {
  localStorage.setItem('bingobongo_autoCheck', autoCheckToggle.checked); // Save auto-check toggle state in localStorage
    ...
    function toggleWinTextVisibility() {
        ...
        if (autoCheckToggle.disabled) {
            winText.textContent = '';  // If the toggle is disabled, make the text blank
        }
        // Check if the toggle is unchecked (and not disabled)
        else if (!autoCheckToggle.checked) {
            winText.textContent = '';  // If unchecked, clear the text inside winText
        } else {
            winText.textContent = 'No Win';  // If checked, show text in winText
        }
    }
