...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Check if there's a saved preference for night mode
    const nightPref = localStorage.getItem('nightMode');

    if (nightPref === null) {
        // No saved preference (first time) → set night mode to ON by default
        body.classList.add('night-mode');
        localStorage.setItem('nightMode', 'on'); // Save 'on' preference for future visits
    } else if (nightPref === 'on') {
        // If the saved preference is 'on', enable night mode
        body.classList.add('night-mode');
    } else if (nightPref === 'off') {
        // If the saved preference is 'off', disable night mode
    ...
    nightModeBtn.addEventListener('click', () => {
        ...
        // Save the current night mode state in localStorage
