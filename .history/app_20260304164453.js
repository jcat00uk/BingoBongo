...
function renderModalCardList() {
  ...
  pinned.forEach(code => {
    ...
    if (modalSelections.has(code)) div.classList.add('selected'); // <-- apply selected class

    div.textContent = code;
    div.onclick = () => handleModalCardClick(code, !modalSelections.has(code));
  ...
  // Other (not pinned, filtered by search)
...
function handleModalCardClick(code, checked) {
  if (checked) {
  ...
  // Re-render the modal to update classes
  renderModalCardList();
}
