...
function populateCardList(filter = '') {
  ...
  sortedCards.forEach(c => {
    ...
    // === LABEL with checkbox + card code only ===
    ...
    // === Preview button toggle: only one open at a time ===
    ...
    checkbox.addEventListener('change', () => {
      ...
      searchInput.value = '';
      populateCardList(searchInput.value); // refresh and snap selected to top
...
clearSearchBtn.addEventListener('click', () => {
  ...
  // Auto expand cardList width if preview opens
if(previewState[c.code]) {
  const previewWidth = previewTable.scrollWidth + 16; // add some padding
  cardList.style.width = previewWidth + 'px';
} else {
  cardList.style.width = 'fit-content';
}
});
