...
// === Preview button outside label, left of checkbox ===
...
infoBtn.textContent = 'ℹ️';
infoBtn.className = 'preview-btn';
infoBtn.addEventListener('click', e => { e.stopPropagation(); togglePreview(c.code); });

// label with checkbox and card id
const label = document.createElement('label');
const checkbox = document.createElement('input');
checkbox.type = 'checkbox';
checkbox.checked = tempSelection.includes(c.code);
...
// Append in order: button first, label fills remaining space
