...
function renderModalCardList() {
  ...
  pinned.forEach(code => {
    const div = document.createElement('div');
    div.className = 'modal-card-item selected'; // already selected
...
function handleModalCardClick(code, div) {
  ...
  if (isSelected) {
    ...
    setTimeout(() => div.classList.remove('deselected'), 300);
  ...
  // Re-render to update other items (optional, if you want consistent styling)
  renderModalCardList();
  updateAutoCheckToggle();
  saveGameState();
}
