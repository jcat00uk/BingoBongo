...
function undoNumber() {
    ...
    // Clear winning cards if their win is undone
    if (firstLineCalled) {
        const [lineCard] = lastLineCards;
        if (!checkLine(cards[lineCard])) {
            firstLineCalled = false;
            lastLineCards.clear();
        }
    }
    if (firstFullHouseCalled) {
        const [fhCard] = lastFullHouseCards;
        if (!checkFullHouse(cards[fhCard])) {
    ...
    recalcFirstWins(); // recalc first wins only for cards that now qualify
    updateRemaining();
...
function recalcFirstWins() {
    // Start with the current win text so we don't overwrite previous wins
    ...
    // Update the text without overwriting previous wins on refresh
