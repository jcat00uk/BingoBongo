...
const winTextOutput = document.getElementById('winTextOutput');  // winTextOutput
...
function toggleWinTextVisibility() {
    ...
    if (autoCheckToggle.disabled || !autoCheckToggle.checked) {
        winText.textContent = '';  // If the toggle is disabled OR unchecked, make the text blank
    } else {
        winText.textContent = winTextOutput.textContent;  // If the toggle is enabled AND checked, show text in winText
    }
}
...
function checkAllSelectedCards() {
  selectedCards.forEach(code => {
    ...
    if (!firstLineCalled && checkLine(card)) {
      ...
      winTextOutput.textContent = `Bingobongo, Line, ${code}`;
    } 
    // First FULL HOUSE
    else if (!firstFullHouseCalled && checkFullHouse(card)) {
      firstFullHouseCalled = true;
      ...
      winTextOutput.textContent = `Bingobongo, FULL HOUSE, ${code}`;
      ;
    }
    else{
      wwinTextOutput.textContent = "no win";
