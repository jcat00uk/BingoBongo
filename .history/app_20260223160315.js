...
function refreshWinText() {
    if (!autoCheckToggle.checked) {
        ...
        winText.textContent = '';   // USE THE DOM ELEMENT
        return;
    }

    // Call your existing checkAllSelectedCards() logic
    // Make sure it returns { type: 'LINE' | 'FULLHOUSE', code: 'CARD1' } or null/undefined
    const result = checkAllSelectedCards(); 

    if (!result) {
        // No win yet
        winText.textContent = 'No Win';
    } else if (result.type === 'LINE') {
        winText.textContent = `Bingobongo, LINE, ${result.code}`;
    } else if (result.type === 'FULLHOUSE') {
        winText.textContent = `Bingobongo, FULL HOUSE, ${result.code}`;
    }
}
