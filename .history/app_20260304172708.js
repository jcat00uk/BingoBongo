...
function renderModalCardList() {
  ...
  selected.forEach(code => {
    const div = document.createElement('div');
    div.className = 'modal-card-item selected'; // ✅ selected class keeps it green
  ...
  if (selected.length && allCardCodes.length > selected.length) {
  ...
  // Unselected cards
  const unselected = allCardCodes.filter(code => !modalSelections.has(code) && code.toLowerCase().includes(searchTerm));
...
function handleModalCardClick(code) {
  if (modalSelections.has(code)) {
    modalSelections.delete(code); // deselect
  } else {
    modalSelections.add(code); // select
  }

  if (cardSearchBox) cardSearchBox.value = ''; // clear search

  renderModalCardList(); // re-render, selected cards go to top and keep color
  updateAutoCheckToggle();
  saveGameState();
}
