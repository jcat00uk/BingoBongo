...
function recalcFirstWins({ restore = false } = {}) {
    // On restore: keep saved firstLineCalled / firstFullHouseCalled
    if (!restore) {
        firstLineCalled = false;
        firstFullHouseCalled = false;
        lastLineCards.clear();
    ...
    let winTextForDisplay = restore ? winTextOutput : 'No Win';
    ...
    for (const code of selectedCards) {
        ...
        if (!firstLineCalled && checkLine(card)) {
            ...
            if (!restore) winTextForDisplay = `Bingobongo, LINE, ${code}`;
        }
        ...
        if (!firstFullHouseCalled && checkFullHouse(card)) {
            ...
            if (!restore) winTextForDisplay = `Bingobongo, FULL HOUSE, ${code}`;
        }
    }

    winTextOutput = restore ? winTextOutput : winTextForDisplay;
    toggleWinTextVisibility();
    saveGameState();
}
...
function loadGameState() {
  ...
  recalcFirstWins({ restore: true }); // restore the previous win without re-announcing
  toggleWinTextVisibility();
  updateRemaining();
  updateCalledNumbersDisplay();
  updateBigLastNumber();
