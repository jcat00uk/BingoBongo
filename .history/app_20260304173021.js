...
function renderModalCardList() {
  ...
  const searchFilter = (cardSearchBox?.value || '').toLowerCase();

  // Selected cards first
  Object.keys(cards)
    .filter(code => modalSelections.has(code))
    .forEach(code => {
      const div = document.createElement('div');
  ...
  // Unselected cards with search filter
  Object.keys(cards)
    .filter(code => !modalSelections.has(code) && code.toLowerCase().includes(searchFilter))
    .forEach(code => {
      const div = document.createElement('div');
...
function handleModalCardClick(code) {
  ...
  const cardDiv = document.querySelector(`.modal-card-item[data-code="${code}"]`);
  if (cardDiv) {
    cardDiv.classList.add(isSelected ? 'flash-red' : 'flash-green');
    setTimeout(() => {
      cardDiv.classList.remove('flash-red', 'flash-green');
    }, 200);
  }
}
