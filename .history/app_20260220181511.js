...
// Ensure that your JavaScript runs after the DOM has loaded.
window.addEventListener('DOMContentLoaded', () => {
    const nightModeBtn = document.getElementById('nightModeBtn');
    const body = document.body;

    // Check the saved preference for night mode in localStorage
    const nightPref = localStorage.getItem('nightMode');

    if (nightPref === 'on') {
        // Apply night mode if the saved preference is 'on'
        body.classList.add('night-mode');
        if (nightModeBtn) {
            nightModeBtn.textContent = '🌙'; // night mode is on
        }
    } else if (nightPref === 'off') {
        // Apply light mode if the saved preference is 'off'
        body.classList.remove('night-mode');
        if (nightModeBtn) {
            nightModeBtn.textContent = '🌞'; // night mode is off
        }
    } else {
        // Default behavior for first-time visitors: Enable night mode by default
        body.classList.add('night-mode');
        localStorage.setItem('nightMode', 'on'); // Save 'on' for future visits
        if (nightModeBtn) {
            nightModeBtn.textContent = '🌙'; // night mode is on by default
        }
    }

    // Toggle the night mode when the button is clicked
