...
// Only scroll if the search box has a value and there are selected cards
if (cardSearchBox && cardSearchBox.value.trim() !== '') {
    ...
    if (selectedCardsInDOM.length > 0) {
        ...
        // Scroll just enough so the last selected card is fully visible
        const containerTop = modalCardList.scrollTop;
        const containerBottom = containerTop + modalCardList.clientHeight;
        const cardBottom = lastSelected.offsetTop + lastSelected.offsetHeight;
        
        if (cardBottom > containerBottom) {
            // Smooth scroll to reveal last selected card
            modalCardList.scrollTo({ top: cardBottom - modalCardList.clientHeight, behavior: 'smooth' });
        }
    }
}
