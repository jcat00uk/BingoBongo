...
document.addEventListener('DOMContentLoaded', function () {
    // Get references to the autoCheckToggle and winText elements
    const autoCheckToggle = document.getElementById('autoCheckToggle');  // The checkbox or toggle switch
    const winText = document.getElementById('winText');  // The div with id="winText"

    // Function to handle clearing or showing the winText based on the toggle
    function toggleWinTextVisibility() {
        // Check the state of the toggle
        if (autoCheckToggle.checked) {
            winText.textContent = 'No Win'; // Set default text when autoCheckToggle is on
        } else {
            winText.textContent = ''; // Clear text when autoCheckToggle is off
        }
    }

    // Add the event listener to monitor changes on the autoCheckToggle
    autoCheckToggle.addEventListener('change', toggleWinTextVisibility);

    // Call the function initially to set the state on page load based on the toggle
