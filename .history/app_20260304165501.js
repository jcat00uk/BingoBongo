...
function renderModalCardList() {
  ...
  pinned.forEach(code => {
    const div = document.createElement('div');
    div.className = 'modal-card-item selected'; // persist selected
    div.textContent = code;
    div.style.textAlign = 'center';
    div.onclick = () => handleModalCardClick(code, div);
...
function handleModalCardClick(code, cardDiv) {
  ...
  if (isSelected) {
    ...
    cardDiv.classList.remove('flash-select'); // reset any previous
    void cardDiv.offsetWidth; // force reflow
    cardDiv.classList.add('flash-deselect'); // flash red
  } else {
    // Add to selection
    modalSelections.add(code);
    cardDiv.classList.add('selected'); // persist green
    cardDiv.classList.remove('flash-deselect'); // reset any previous
    void cardDiv.offsetWidth;
    cardDiv.classList.add('flash-select'); // flash green
  }
  ...
  setTimeout(() => {
    cardDiv.classList.remove('flash-select', 'flash-deselect');
  }, 400);
}
