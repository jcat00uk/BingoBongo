...
// Function to handle hiding or showing the winText
function toggleWinTextVisibility() {
    if (autoCheckToggle.checked) {
        winText.style.display = 'block'; // Show the winText div if autoCheckToggle is on
    } else {
        winText.style.display = 'none'; // Hide the winText div if autoCheckToggle is off
    }
}

// 3. Add the event listener to monitor changes on the autoCheckToggle
autoCheckToggle.addEventListener('change', toggleWinTextVisibility);

// 4. Call the function initially to set the state on page load
